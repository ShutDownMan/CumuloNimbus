...
extern int64_t _set_time_window(int64_t dataseries_id, uint64_t time_window);
...
int64_t compute(int64_t dataseries_a_id, int64_t dataseries_b_id) {
    ...
    A = _set_time_window(A, (uint64_t)1 * 60 * 1000 * 1000000);

    int64_t B = _copy_dataseries(dataseries_b_id);
    B = _set_interpolation_strategy(B, 1);
    B = _set_time_window(B, (uint64_t)1 * 60 * 1000 * 1000000);

    int64_t result = _add(A, B);
    result = _set_interpolation_strategy(result, 1);
